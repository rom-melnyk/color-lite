 ...
 */
Convert.rgb2hsl = function (rgb) {
	var r = normalizeChannel('r', rgb.r) / 255,
		g = normalizeChannel('g', rgb.g) / 255,
		b = normalizeChannel('b', rgb.b) / 255,
		max = Math.max(r, g, b),
		min = Math.min(r, g, b),
		delta = max - min,
		hsl = {};
 ...
 */
Convert.hsl2rgb = function (hsl) {
	var h = normalizeChannel('h', hsl.h),
		s = normalizeChannel('s', hsl.s) / 100,
		l = normalizeChannel('l', hsl.l) / 100,
