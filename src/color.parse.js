 ...
 */
function getAlternativeFormat (format) {
 ...
 */
function parseHexRgb (content) {
	var ret = ['rgbhex'],
		parsed = masks.rgbhex.exec(content),
		i;
...
function parse (format, content) {
	...
	if (typeof content === 'object') {
		keys.forEach(function (key) {
			ret[key] = normalizeChannel( key, content[key] );
		});
	} else if (typeof content === 'string') {
		var parsedString = masks[format].exec(content)
			|| masks[ getAlternativeFormat(format) ].exec(content);
		...
		keys.forEach(function (key) {
			ret[key] = normalizeChannel( key, parsedString[i++] );
		});
	} else {
		var args = Array.prototype.slice.call(arguments, 1);
		keys.forEach(function (key) {
			ret[key] = normalizeChannel( key, args[i++] );
		});
	}
